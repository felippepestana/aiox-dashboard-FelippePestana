    ...
    <div className="min-h-screen bg-[#F5F5F7] p-6">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-[#86868B] mt-1">Performance de marketing digital</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex rounded-xl bg-white border border-[#E5E5EA] overflow-hidden">
            {(['week', 'month', 'quarter'] as const).map((p) => (
              <button key={p} onClick={() => setPeriod(p)}
                className={`px-4 py-2 text-xs font-medium transition-all ${period === p ? 'bg-[#C4956A]/20 text-[#C4956A]' : 'text-[#86868B] hover:text-[#1D1D1F]'}`}>
        ...
        <div className="col-span-2 rounded-2xl bg-white border border-[#E5E5EA] p-6">
          ...
          <div className="grid grid-cols-4 gap-3">
            {CHANNEL_STATS.map((ch) => {
              const Icon = ch.icon;
              return (
                <div key={ch.name} className="rounded-xl bg-[#FAFAFA] border border-[#E8E8ED] p-4 hover:border-[#D1D1D6] transition-all">
                  <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${ch.bg} mb-3`}>
                    <Icon className={`h-5 w-5 ${ch.color}`} />
                  </div>
                  <p className="text-sm font-semibold text-[#1D1D1F]">{ch.name}</p>
                  <div className="mt-3 space-y-2">
                    <div className="flex justify-between text-[10px]">
                      <span className="text-[#AEAEB2]">Seguidores</span>
                      <span className="text-[#1D1D1F]">{ch.followers}</span>
                    </div>
                    <div className="flex justify-between text-[10px]">
                      <span className="text-[#AEAEB2]">Engajamento</span>
                      <span className="text-[#C4956A]">{ch.engagement}</span>
                    </div>
                    <div className="flex justify-between text-[10px]">
                      <span className="text-[#AEAEB2]">Posts/Mês</span>
                      <span className="text-[#1D1D1F]">{ch.posts}</span>
                    </div>
                  </div>
                </div>
              );
            ...
            {WEEKLY_REACH.map((d) => {
              ...
              return (
                <div key={d.day} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-[9px] text-[#AEAEB2] font-mono">{(d.value / 1000).toFixed(1)}k</span>
                  ...
                  <span className="text-[9px] text-[#AEAEB2]">{d.day}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 rounded-2xl bg-white border border-[#E5E5EA] p-6">
          ...
          <div className="space-y-2">
            {TOP_CONTENT.map((content, i) => (
              <div key={i} className="flex items-center gap-4 rounded-xl bg-[#FAFAFA] border border-[#E8E8ED] p-3 hover:border-[#D1D1D6] transition-all">
                <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-pink-500/10 text-xs font-bold text-pink-400">
                  {i + 1}
                </span>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-[#1D1D1F] truncate">{content.title}</p>
                  <div className="flex items-center gap-3 mt-0.5">
                    <span className="text-[10px] text-[#AEAEB2]">{content.platform}</span>
                    <span className="text-[10px] text-[#AEAEB2]">{content.type}</span>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-xs font-medium text-[#1D1D1F]">{content.reach}</p>
                  <p className="text-[10px] text-[#C4956A]">{content.engagement} engaj.</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl bg-white border border-[#E5E5EA] p-5">
            <h3 className="text-sm font-semibold text-[#1D1D1F] mb-4">Ações de Marketing</h3>
            <div className="space-y-2">
              {[
