...
const STATUS_MAP: Record<string, { label: string; color: string; bg: string; icon: typeof Clock }> = {
  ...
  delivered: { label: 'Entregue', color: 'text-[#C4956A]', bg: 'bg-[#C4956A]/10 border-[#C4956A]/20', icon: Download },
};

const URGENCY_MAP: Record<string, { label: string; color: string }> = {
  routine: { label: 'Rotina', color: 'text-[#86868B]' },
    ...
    <div className="min-h-screen bg-[#F5F5F7] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[#86868B] mt-1">Gerenciamento de exames e resultados conforme normas CFO/ANS</p>
        </div>
        <button
          ...
          return (
            <button
              ...
              className={`rounded-xl border p-3 transition-all ${
                filterStatus === key ? conf.bg : 'bg-white border-[#E5E5EA] hover:border-[#D1D1D6]'
              }`}
            >
              <div className="flex items-center gap-2">
                <Icon className={`h-4 w-4 ${filterStatus === key ? conf.color : 'text-[#AEAEB2]'}`} />
                <span className={`text-lg font-bold ${filterStatus === key ? conf.color : 'text-[#1D1D1F]'}`}>
                  {statusCounts[key] || 0}
                </span>
              </div>
              <p className="text-[10px] text-[#86868B] mt-1">{conf.label}</p>
            </button>
          );
        })}
      </div>
      ...
      <div className="space-y-3">
        {filtered.map((exam) => {
          ...
          return (
            <div key={exam.id} className="rounded-2xl bg-white border border-[#E5E5EA] p-5 hover:border-[#C4956A]/10 transition-all">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <p className="text-sm font-semibold text-[#1D1D1F]">{exam.examType}</p>
                  ...
                  <div className="flex items-center gap-6 mb-2">
                    <span className="text-xs text-[#6E6E73] flex items-center gap-1.5">
                      <span className="text-[#AEAEB2]">Paciente:</span> {exam.patientName}
                    </span>
                    <span className="text-xs text-[#6E6E73] flex items-center gap-1.5">
                      <span className="text-[#AEAEB2]">Região:</span> {exam.region}
                    </span>
                  </div>
                  <p className="text-xs text-[#86868B] leading-relaxed">
                    <span className="text-[#AEAEB2]">Justificativa:</span> {exam.justification}
                  </p>
                  <div className="flex items-center gap-4 mt-3 text-[10px] text-[#AEAEB2]">
                ...
                <div className="flex gap-2 ml-4">
                  {(exam.status === 'completed' || exam.status === 'delivered') && (
                    <button className="flex items-center gap-1.5 rounded-lg bg-[#C4956A]/10 border border-[#C4956A]/20 px-3 py-1.5 text-[10px] font-medium text-[#C4956A] hover:bg-[#C4956A]/20 transition-all">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-2xl bg-white border border-[#E5E5EA] p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowNewExam(false)} className="text-[#AEAEB2] hover:text-[#1D1D1F]">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div>
                <label className="text-xs font-medium text-[#6E6E73] mb-2 block">Tipo de Exame</label>
                <div className="grid grid-cols-2 gap-2">
                  {EXAM_TYPES.map((exam) => (
                    <button
                      key={exam.id}
                      className="flex items-center gap-3 rounded-xl bg-[#FAFAFA] border border-[#E8E8ED] p-3 hover:border-[#C4956A]/30 transition-all text-left"
                    >
                      <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-500/10">
                        <ImageIcon className="h-4 w-4 text-amber-400" />
                      </div>
                      <div>
                        <p className="text-xs font-medium text-[#1D1D1F]">{exam.name}</p>
                        <p className="text-[10px] text-[#AEAEB2]">{exam.code} - {exam.category}</p>
                      </div>
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="text-xs font-medium text-[#6E6E73] mb-1 block">Região Anatômica</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-medium text-[#6E6E73] mb-1 block">Justificativa Clínica</label>
                <textarea
                  rows={3}
              ...
              <div>
                <label className="text-xs font-medium text-[#6E6E73] mb-2 block">Urgência</label>
                <div className="flex gap-3">
                  {[
                    ...
                    <button
                      key={u.value}
                      className="flex-1 rounded-xl bg-[#FAFAFA] border border-[#E8E8ED] p-3 hover:border-[#C4956A]/30 transition-all text-center"
                    >
                      <p className="text-xs font-medium text-[#1D1D1F]">{u.label}</p>
                      <p className="text-[10px] text-[#AEAEB2]">{u.desc}</p>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowNewExam(false)} className="flex-1 rounded-xl bg-[#E5E5EA] px-4 py-2.5 text-sm text-[#6E6E73] hover:text-[#1D1D1F] transition-all">
                Cancelar
              </button>
              <button onClick={() => setShowNewExam(false)} className="flex-1 rounded-xl bg-gradient-to-r from-[#C4956A] to-[#A0784C] px-4 py-2.5 text-sm font-medium text-white hover:shadow-lg hover:shadow-[#C4956A]/20 transition-all">
                Solicitar Exame
              </button>
            </div>

            <p className="text-[9px] text-[#AEAEB2] text-center mt-4">
